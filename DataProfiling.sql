...
--	Matt Braithwaite
--	29Jan24
--	Queries for testing tables/columns

--	This file contains two distinct sets of code I frequently use from online resources.
--	a) syscolumbns/sysobject search
--		(https://blog.sqlauthority.com/2021/02/25/sql-server-get-column-names/)
--	b) data profiling script i've pulled from an online resource to review the data in scope. 
--		(https://www.sqlservercentral.com/articles/data-profiling-with-t-sql)
...
--	Identify matching column names to confirm relationships exist 
--	(assumes same name on both sides of the relationship)
SELECT		SCHEMA_NAME(o.schema_id) SchemaName, o.Name, c.Name, c.system_type_id, t.name DataType
FROM		sys.columns c 
			inner jOIN sys.objects o ON o.object_id = c.object_id 
			INNER JOIN sys.types t ON c.user_type_id = t.user_type_id
WHERE		o.type = 'U' 
AND			c.name like '%recent%'
AND			o.Name NOT LIKE '%all'
--AND		o.NAme IN ('medication','clinical_codes')
...
USE EMIS                                        -- Your database here
GO


DECLARE	@TABLE_SCHEMA NVARCHAR(128) = 'dbo'         -- Your schema here
DECLARE	@TABLE_NAME NVARCHAR(128) = 'patient'        -- Your table here
